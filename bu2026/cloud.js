...
const SESSION_KEY="BU2026_SB_SESSION_V2";
...
let session=null,user=null,profile=null,cloudReady=false,locks=[],currentClaim=null;
let pollTimer=null,heartbeatTimer=null,toastTimer=null,patched=false,lastCloudFingerprint="";
...
const setQueue=q=>{localStorage.setItem(QUEUE_KEY,JSON.stringify(q));renderCloudStatus()};
const saveSession=s=>{session=s||null;if(s)localStorage.setItem(SESSION_KEY,JSON.stringify(s));else localStorage.removeItem(SESSION_KEY)};
const loadSession=()=>{try{return JSON.parse(localStorage.getItem(SESSION_KEY)||"null")}catch{return null}};

function toast(msg,type=""){
 const e=$("cloudToast");if(!e)return;
 e.textContent=msg;e.className="cloud-toast show"+(type?" "+type:"");
 clearTimeout(toastTimer);toastTimer=setTimeout(()=>e.className="cloud-toast",4200);
}
function authMsg(msg,type="warn"){
 const e=$("cloudAuthMsg");if(!e)return;
 e.textContent=msg;e.className="cloud-msg show "+type;
}
function setAuthBusy(v){
 ["cloudLoginBtn","cloudSignupBtn","cloudBootstrapBtn"].forEach(id=>{const e=$(id);if(e)e.disabled=v});
}
...
async function fetchJSON(url,opts={},timeout=16000){
 ...
 try{
  ...
  if(!r.ok){
   const msg=data?.msg||data?.message||data?.error_description||data?.error||("HTTP "+r.status);
   const err=new Error(msg);err.status=r.status;err.data=data;throw err;
  }
  return {data,response:r};
 }catch(e){
  if(e.name==="AbortError")throw new Error("Tempo de conexão esgotado. Verifique a internet.");
  throw e;
 }finally{clearTimeout(timer)}
}
async function authRequest(path,body){
 return (await fetchJSON(SB_URL+"/auth/v1/"+path,{
  method:"POST",headers:headers(false),body:JSON.stringify(body||{})
 })).data;
}
async function rest(path,{method="GET",body=null,prefer="",auth=true}={}){
 const extra={};if(prefer)extra.Prefer=prefer;
 const opts={method,headers:headers(auth,extra)};
 if(body!==null)opts.body=JSON.stringify(body);
 return fetchJSON(SB_URL+"/rest/v1/"+path,opts);
}
async function rpc(name,args={}){
 return (await rest("rpc/"+name,{method:"POST",body:args,prefer:"return=representation"})).data;
}
async function select(table,query=""){
 return (await rest(table+(query?("?"+query):""))).data;
}
async function insert(table,row){
 return (await rest(table,{method:"POST",body:row,prefer:"return=representation"})).data;
}
async function update(table,query,patch){
 return (await rest(table+"?"+query,{method:"PATCH",body:patch,prefer:"return=representation"})).data;
}
...
async function refreshSessionIfNeeded(){
 ...
 if(expiresAt&&expiresAt-now>120)return session;
...
function ensureUI(){
 ...
 if(sidebar){
  ...
  grow?.insertAdjacentHTML("beforebegin",
   '<div id="cloudStatusCard" class="cloud-status offline" title="Clique para ver a sincronização"><b><span class="dot"></span> Nuvem</b><span id="cloudStatusText">Conectando…</span></div>'+
   '<button id="cloudTeamBtn" class="side-link" style="display:none"><span class="side-icon">☁</span><span>Equipe & Auditoria</span></button>');
 }
...
async function login(){
 ...
 if(!email||!password){authMsg("Informe e-mail e senha.","bad");return}
 setAuthBusy(true);authMsg("Entrando…","warn");
 try{
  const s=await authRequest("token?grant_type=password",{email,password});
  saveSession(s);await activate(s.user);
 }catch(e){authMsg(e.message,"bad")}
...
async function signup(){
 ...
 try{
  const s=await authRequest("signup",{email,password,data:{display_name}});
  if(s?.access_token&&s?.user){
   saveSession(s);await activate(s.user);
  }else{
   authMsg("Cadastro criado. Se o Supabase exigir confirmação por e-mail, confirme e depois entre. Novos usuários aguardam liberação do administrador.","good");
  }
 }catch(e){authMsg(e.message,"bad")}
 finally{setAuthBusy(false)}
}
async function logout(){
 ...
 try{if(navigator.onLine&&session?.access_token)await fetchJSON(SB_URL+"/auth/v1/logout",{method:"POST",headers:headers(true),body:"{}"})}catch{}
 saveSession(null);stopCloud();showAuth("Sessão encerrada.","good");
}

async function ensureProfile(){
 let rows=await select("bu_profiles","select=*&id=eq."+encodeURIComponent(user.id));
 let p=Array.isArray(rows)?rows[0]:null;
 if(!p){
  await rpc("bu_ensure_profile",{});
  rows=await select("bu_profiles","select=*&id=eq."+encodeURIComponent(user.id));
  p=Array.isArray(rows)?rows[0]:null;
 }
 if(!p)throw new Error("Perfil de acesso não encontrado.");
 return p;
}
async function bootstrapAdmin(){
 const code=$("cloudBootstrapCode").value.trim();
 if(!code){authMsg("Informe o código único de ativação.","bad");return}
 setAuthBusy(true);
 try{
  const data=await rpc("bu_bootstrap_admin",{p_code:code});
  profile=Array.isArray(data)?data[0]:data;
  localStorage.setItem(PROFILE_CACHE_KEY,JSON.stringify(profile));
  $("cloudBootstrapBox").classList.remove("show");
  authMsg("Administrador ativado com sucesso.","good");
  await activate(user);
 }catch(e){authMsg(e.message==="invalid bootstrap code"?"Código de ativação inválido.":e.message,"bad")}
 finally{setAuthBusy(false)}
}
...
async function activate(u){
 ...
 try{
  ...
  showAuth("Não foi possível validar seu acesso: "+e.message,"bad");return;
 }
 ...
 if(!profile.active){
  let bootstrap=null;
  if(navigator.onLine){
   try{bootstrap=await rpc("bu_bootstrap_status",{})}catch{}
  }
  showAuth(bootstrap&&!bootstrap.admin_exists&&bootstrap.code_available?
   "Este é o primeiro acesso administrativo. Informe o código único abaixo.":
   "Seu cadastro existe, mas ainda aguarda liberação do administrador.");
  ...
  $("cloudLoginBtn").onclick=async()=>{try{profile=await ensureProfile();if(profile.active)await activate(user);else authMsg("Acesso ainda pendente.","warn")}catch(e){authMsg(e.message,"bad")}};
  return;
 }

 $("cloudLoginBtn").textContent="Entrar";$("cloudLoginBtn").onclick=login;
 hideAuth();cloudReady=true;
 document.body.classList.toggle("cloud-viewer",profile.role==="viewer");
 $("cloudTeamBtn").style.display=profile.role==="admin"?"flex":"none";
 patchApp();
 ...
 await syncQueue();
 renderCloudStatus();
 toast("Conectado à central DW Tech como "+roleLabel(profile.role));
}

function stopCloud(){
 cloudReady=false;profile=null;user=null;currentClaim=null;
...
function startPolling(){
 ...
 pollTimer=setInterval(async()=>{
  ...
  try{await refreshSessionIfNeeded();await Promise.all([loadCloudBallots(true),loadClaims()])}catch{}
 },4000);
}

function rowToRec(r){
 return{id:r.id,municipio:r.municipio,uf:r.uf,zona:String(r.zona),secao:String(r.secao),local:r.local||"",enderecoLocal:r.endereco_local||"",urna:r.urna||"",dataEleicao:r.data_eleicao||"2026-10-04",aptos:Number(r.aptos||0),comparecimento:Number(r.comparecimento||0),votes:r.votes||{},qrImport:r.qr_import||null,salvoEm:r.created_at,_cloud:true,cloudStatus:r.status,createdBy:r.created_by,revision:r.revision,updatedAt:r.updated_at};
}
async function loadCloudBallots(silent=false){
 ...
 try{
  ...
  const fingerprint=JSON.stringify((data||[]).map(x=>[x.id,x.revision,x.updated_at,x.status]));
  if(silent&&fingerprint===lastCloudFingerprint)return;
  lastCloudFingerprint=fingerprint;

  buData=(data||[]).map(rowToRec);
  const pending=queue().filter(x=>x.state!=="conflict").map(x=>({...x.rec,_queued:true}));
  for(const r of pending)if(!buData.some(b=>b.id===r.id||String(b.secao)===String(r.secao)))buData.unshift(r);

  localStorage.setItem(KEY_BU,JSON.stringify(buData));
  renderAll();renderCloudStatus();
 }catch(e){if(!silent)toast("Falha ao atualizar B.U.s: "+e.message,"bad")}
}
async function loadClaims(){
 if(!cloudReady||!navigator.onLine)return;
 try{locks=await select("bu_section_claims","select=*&expires_at=gt."+encodeURIComponent(new Date().toISOString()))||[];renderSections()}catch{}
}
...
async function claimSection(secao){
 ...
 const data=await rpc("bu_claim_section",{p_secao:Number(secao),p_device_id:DEVICE_ID});
 if(data?.ok){
  currentClaim=String(secao);clearInterval(heartbeatTimer);
  heartbeatTimer=setInterval(()=>heartbeat(),120000);await loadClaims();return data;
 }
 return data||{ok:false,reason:"busy"};
}
async function heartbeat(){
 if(!currentClaim||!navigator.onLine)return;
 try{await rpc("bu_heartbeat_section",{p_secao:Number(currentClaim),p_device_id:DEVICE_ID})}catch{}
}
async function releaseClaim(){
 if(!currentClaim){return}
 const s=currentClaim;currentClaim=null;clearInterval(heartbeatTimer);
 if(!navigator.onLine)return;
 try{await rpc("bu_release_section",{p_secao:Number(s),p_device_id:DEVICE_ID})}catch{}
 await loadClaims();
}
...
function rowFromRec(rec,isUpdate=false){
 ...
 if(!isUpdate){x.id=rec.id;x.election_year=2026;x.created_by=user.id}
 return x;
}
async function saveToCloud(rec){
 ...
 if(existing){
  ...
  await update("bu_ballots","id=eq."+encodeURIComponent(rec.id),rowFromRec(rec,true));return{updated:true};
 }
 ...
 try{
  await insert("bu_ballots",rowFromRec(rec,false));return{inserted:true};
 }catch(e){
  if(e.status===409||String(e.message).includes("duplicate"))throw Object.assign(new Error("Esta seção já existe na central."),{kind:"conflict"});
  throw e;
...
async function syncQueue(){
 ...
 for(const item of q){
  ...
  try{await saveToCloud(item.rec);item.state="done";changed=true}
  catch(e){
 ...
 await loadCloudBallots();
 if(q.some(x=>x.state==="conflict"))toast("Há conflito de sincronização para revisar.","warn");
}
...
async function submitCapture(e){
 ...
 try{
  if(!navigator.onLine){
   enqueue(rec);rec._queued=true;buData.unshift(rec);localStorage.setItem(KEY_BU,JSON.stringify(buData));renderAll();resetBU();toast("Sem internet: B.U. guardado na fila para sincronizar.","warn");return;
  }
  await saveToCloud(rec);await loadCloudBallots();resetBU();toast("B.U. salvo na central e disponível para toda a equipe.");
 }catch(err){
  if(!navigator.onLine||/fetch|network|internet/i.test(err.message||"")){
   enqueue(rec);rec._queued=true;buData.unshift(rec);localStorage.setItem(KEY_BU,JSON.stringify(buData));renderAll();resetBU();toast("Conexão caiu: B.U. ficou na fila de sincronização.","warn");
...
function decorateSections(){
 ...
 [...s.options].forEach(o=>{
  if(!o.value)return;
  const v=String(Number(o.value)),lock=locks.find(x=>String(x.secao)===v&&x.user_id!==user?.id);
...
function patchApp(){
 ...
 const baseReset=resetBU;
 resetBU=function(){releaseClaim();baseReset()};

 const baseEdit=editBU;
 editBU=function(id){
  const r=buData.find(x=>x.id===id);
  if(r?._cloud&&profile?.role!=="admin"){alert("Este B.U. já está finalizado na central. Somente administrador pode fazer correção.");return}
  baseEdit(id);
 };
 ...
 deleteBU=async function(id){
  ...
  if(r._queued){
   if(confirm("Descartar este B.U. da fila offline?")){
    setQueue(queue().filter(x=>x.rec.id!==id));buData=buData.filter(x=>x.id!==id);localStorage.setItem(KEY_BU,JSON.stringify(buData));renderAll();
   }return;
  }
  ...
  try{await update("bu_ballots","id=eq."+encodeURIComponent(id),{status:"anulado",updated_by:user.id});await loadCloudBallots();toast("B.U. anulado. O histórico foi preservado.","warn")}
  catch(e){alert(e.message)}
 };

 const form=$("buForm");form?.addEventListener("submit",submitCapture,true);
 $("secao")?.addEventListener("change",async e=>{
  const v=e.target.value;if(!v||!cloudReady||profile.role==="viewer")return;
  try{
   const r=await claimSection(v);
   if(!r?.ok){alert(r?.reason==="busy"?"Esta seção está em edição por outro usuário.":"Esta seção já foi finalizada.");e.target.value="";fillSection()}
...
async function openAdmin(){
 if(profile?.role!=="admin"){openSyncPane();return}
 $("cloudAdminModal").classList.add("open");selectAdminPane("users");await loadAdminPanel();
}
async function openSyncPane(){
 $("cloudAdminModal").classList.add("open");selectAdminPane("sync");await renderSyncPane();
}
async function loadAdminPanel(){
 if(profile?.role!=="admin"){await renderSyncPane();return}
 try{
  const [users,audit,ballots]=await Promise.all([
   select("bu_profiles","select=*&order=created_at.asc"),
   select("bu_audit_logs","select=*&order=created_at.desc&limit=120"),
   select("bu_ballots","select=id&status=eq.finalizado")
  ]);
  const active=(users||[]).filter(x=>x.active).length,pending=(users||[]).filter(x=>!x.active).length;
  $("cloudSummary").innerHTML='<div><span>Usuários</span><b>'+users.length+'</b></div><div><span>Ativos</span><b>'+active+'</b></div><div><span>Pendentes</span><b>'+pending+'</b></div><div><span>B.U.s centrais</span><b>'+ballots.length+'</b></div>';

  $("cloudPaneUsers").innerHTML='<div class="cloud-user-row head"><span>Usuário</span><span>Nome</span><span>Perfil</span><span>Ativo</span><span>Ação</span></div>'+
   users.map(x=>'<div class="cloud-user-row" data-user="'+x.id+'"><span>'+esc(x.email||"")+'<small>'+esc(x.id.slice(0,8))+'</small></span><input class="cu-name" value="'+esc(x.display_name||"")+'"><select class="cu-role"><option value="viewer"'+(x.role==="viewer"?" selected":"")+'>Consulta</option><option value="operator"'+(x.role==="operator"?" selected":"")+'>Operador</option><option value="admin"'+(x.role==="admin"?" selected":"")+'>Administrador</option></select><label><input class="cu-active" type="checkbox"'+(x.active?" checked":"")+'> sim</label><button class="primary cu-save">Salvar</button></div>').join("");

  document.querySelectorAll(".cu-save").forEach(b=>b.onclick=async()=>{
   const row=b.closest(".cloud-user-row"),id=row.dataset.user,role=row.querySelector(".cu-role").value,active=row.querySelector(".cu-active").checked,name=row.querySelector(".cu-name").value.trim();
   b.disabled=true;
   try{await rpc("bu_admin_set_access",{p_user_id:id,p_role:role,p_active:active,p_display_name:name});toast("Acesso atualizado.");await loadAdminPanel()}
   catch(e){alert(e.message)}finally{b.disabled=false}
  });

  $("cloudPaneAudit").innerHTML=(audit||[]).map(a=>'<div class="audit-row"><b>Seção '+String(a.secao).padStart(3,"0")+'</b><span>'+esc(a.action)+'</span><span>'+esc(a.actor_email||a.user_id||"sistema")+'</span><span>'+new Date(a.created_at).toLocaleString("pt-BR")+'</span></div>').join("")||'<div class="cloud-empty">Sem eventos de auditoria.</div>';
  await renderSyncPane();
 }catch(e){toast(e.message,"bad")}
}
async function renderSyncPane(){
 const q=queue();
 $("cloudPaneSync").innerHTML='<div class="actions" style="margin-bottom:10px"><button id="syncNowBtn" class="primary">↻ Sincronizar agora</button><button id="logoutCloudBtn" class="soft">Sair deste usuário</button></div>'+
 ...
 $("logoutCloudBtn").onclick=logout;
 document.querySelectorAll(".discard-sync").forEach(b=>b.onclick=()=>{
  if(confirm("Descartar esta pendência local?")){setQueue(queue().filter(x=>x.id!==b.dataset.id));renderSyncPane();loadCloudBallots()}
 });
}
...
async function boot(){
 ...
 try{
  ...
  showAuth("Falha ao iniciar a central: "+e.message,"bad");
 }

 window.addEventListener("online",async()=>{
  renderCloudStatus();
  if(cloudReady){try{await refreshSessionIfNeeded();await syncQueue();await loadCloudBallots()}catch{}}
 });
 window.addEventListener("offline",renderCloudStatus);
}

window.BUCloud={get profile(){return profile},get ready(){return cloudReady},reload:loadCloudBallots,sync:syncQueue,logout,openAdmin};
